        ...
        for link in soup.select('.TableText a'):
            ...
            if len(body.split('Permanent Address :')) > 1:
                ...
                if len(address) > 2:
                    ...
                    c.execute("INSERT INTO users_user (roll,name,email,username) VALUES ("+roll+",'"+name+"','"+email+"','"+username+"') ON CONFLICT(roll) DO UPDATE SET name=Excluded.name,email=Excluded.email,username=Excluded.username")
